			...
			<div class="column is-12">
                <div class="post-header mb-5 ">
                        <div class="meta-cat text-color">
                            ...
                            <a class="post-category font-extra text-color text-uppercase font-sm letter-spacing-1" href="<?=base_url("/Naskah/Digital");?>">Naskah Digital</a>
                        </div>
                    </div>
				<div class="columns is-multiline">
                <?php
                    foreach($naskah as $value):
						...
						echo "<div class=\"column is-3-desktop is-6-tablet\">
									<article class=\"post-grid mb-6\">
                                        <div class=\"post-thumb mb-5 is-block _df_thumb\" download=\"\" source=\"{$pdf}\" thumb=\"{$thumb}\"></div>
                                        <h4 class=\"cat-name text-color font-extra text-sm is-uppercase letter-spacing-1\">".$value['title']."</h4>								
									</article>
								</div>";
                    endforeach;
                ?>
				...
				<?= $pager->links('naskah', 'bootstrap_pagination'); ?>
                </div>
			</div>
		</div>
	</div>
...
<script type="text/javascript">
      ...
      $(document).ready(function () {
          $(".pdfflip").on('click', function (event){ 
            
            $(this).flipBook({
                //Layout Setting
                pdfUrl: $(this).attr('value'),
                lightBox:true,
