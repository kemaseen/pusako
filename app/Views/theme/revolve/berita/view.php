...
<section class="single-block-wrapper section-padding">
	<div class="container">
		<div class="columns is-desktop">
				...
				<div class="single-post">
                    <div class="post-header mb-5 ">
                        <div class="meta-cat text-color">
                            ...
                            <a class="post-category font-extra text-color text-uppercase font-sm letter-spacing-1" href="<?=base_url("/Berita");?>">Berita</a>
                        </div>
                        <h2 class="post-title mt-2">
                        <?=$news['title'];?>
                        </h2>
                        <div class="post-meta">
                            <span class="text-uppercase font-sm letter-spacing-1 mr-3">Reporter : <?=$news['author'];?></span>
                            <span class="text-uppercase font-sm letter-spacing-1 mr-3">Editor : <?=$news['author'];?></span>
                            <span class="text-uppercase font-sm letter-spacing-1"><?=date("l, d F Y - H:i", strtotime($news['created_at']));?></span>
                        </div>
                    </div>
                    <div class="post-body">
                        ...
                        <div class="post-tags py-4">
                            <?php if(trim($news['tag']) != ''){
                                    $tags = explode("#", trim($news['tag']));
                                    foreach($tags as $value){
                                        if(trim($value) != '')
                                            echo "<a href=\"".base_url('Berita/tag/#'.$value)."\">#".$value."</a>";
                                    }
                               }
                            ?>
                        </div>
			...
			<?= view_cell('\App\Libraries\Widget::berita', ['title' => $news['title'], 'limit' => 10]) ?>
		</div>
	</div>
</section>
<?= $this->endSection(); ?>
